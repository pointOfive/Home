...
def ucb1(self):
    ...
    Return the index of the winning bandit.
    '''
    return np.argmax(.01+self.wins/(.01+self.trials) + (2*np.log(sum(.01+self.trials))/(01.+np.array(self.trials)))**.5)
...
if __name__ == "__main__":
    ...
    for k in range(3):
        ...
        plt.plot(x, tot_wins_all,'ko',alpha=.1)
        cur = 0
        for i in range(T):
            if np.isnan(tot_wins_all[i]):
                ...
                cur = tot_wins_all[i]
        plt.plot(x, tot_wins_all,label="bandit "+str(k)+": " + str(bandits.p_array[k]))
    plt.ylim([-.01,.1])
    plt.legend()
    plt.show()
